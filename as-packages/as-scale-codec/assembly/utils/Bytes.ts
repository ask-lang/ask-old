...
export class Bytes {
    ...
    static toUint<T extends number>(
        b: u8[],
        bitLength: i32,
        index: i32 = 0
    ): T {
    ...
    static appendUint<T extends number>(
        b: Array<u8>,
        v: T,
        bitLength: i32
    ): void {
    ...
    static decodeCompactInt(input: u8[], index: i32 = 0): DecodedData<u64> {
        ...
        if (i32(byteLen) == BIT_LENGTH.INT_32) {
            return new DecodedData<u64>(
                u64(Bytes.toUint<u32>(buf, BIT_LENGTH.INT_32)),
                BIT_LENGTH.INT_32
            );
        }

        if (
            i32(byteLen) > BIT_LENGTH.INT_32 &&
            i32(byteLen) < BIT_LENGTH.INT_64
        ) {
            const tmp = new Array<u8>(8);
            Bytes.copy<u8>(buf, tmp);
            return new DecodedData<u64>(
                Bytes.toUint<i64>(tmp, BIT_LENGTH.INT_64),
                BIT_LENGTH.INT_64
            );
        }
    ...
    static decodeSmallInt(
        ...
        assert(
            ...
            "Small Int: mode is invalid"
        );
        if (mode == 0) {
            return new DecodedData<u64>(
                u64(Bytes.decodeByte(input[index])),
                BIT_LENGTH.INT_8
            );
        } else if (mode == 1) {
            assert(
                i32(input.length - index) >= BIT_LENGTH.INT_16,
                "Invalid input: expected 2 bytes array"
            );
            return new DecodedData<u64>(
                u64(Bytes.decode2Bytes([input[index], input[index + 1]])),
                BIT_LENGTH.INT_16
            );
        } else {
            assert(
                i32(input.length - index) >= BIT_LENGTH.INT_32,
                "Invalid input: expected 4 bytes array"
            );
            return new DecodedData<u64>(
                u64(
                    Bytes.decode4Bytes([
